   ...
   for (i = 0; i < num_properties; i++)
   {
      ...
      if (a->name & GUID_EXISTS_FLAG)
      {
         ...
         if (a->num_names > 0)
         {
            ...
            for (k = 0; k < a->num_names; k++)
            {
               size_t j;

               a->names[k].len = GETINT32(buf + idx); idx += 4;

               /* read the data into a buffer */
               a->names[k].data
                  = CHECKED_XMALLOC(unsigned char, a->names[k].len);
               for (j = 0; j < (a->names[k].len >> 1); j++)
                  a->names[k].data[j] = (buf + idx)[j * 2];

               /* But what are we going to do with it? */

               idx += pad_to_4byte(a->names[k].len);
            }
         }
         else
         {
